...
class Test01UserAPI:
    ...
    def test_04_users_get_auth(self, user_client, admin):
        ...
        data = response.json()
        assert type(data) == list, "Возврщаемый тип данных`/api/v1/users/` должен быть списком"
        assert len(data) == 1, "Неверное количество возвращаемых данных."
        assert data[0].get('username') == admin.username and data[0].get('email') == admin.email, \
            "Проверьте правильность возвращаемых данных"
    ...
    def test_05_users_post_auth(self, user_client, admin):
        ...
        data = response.json()
        assert len(data) == 3, \
            'Проверьте, что при GET запросе `/api/v1/users/` возвращаете правильные данные. ' \
            'Количество возвращаемых данных не соответствует действительному.'

    @pytest.mark.django_db(transaction=True)
    def test_06_users_username_get_auth(self, user_client, admin):
        user, moderator = create_users_api(user_client)
        response = user_client.get(f'/api/v1/users/{admin.email}/')
        assert response.status_code != 404, \
            'Страница `/api/v1/users/{username}/` не найдена, проверьте этот адрес в *urls.py*'
        assert response.status_code == 200, \
        ...
        response = user_client.get(f'/api/v1/users/{moderator.email}/')
        assert response.status_code == 200, \
            'Проверьте, что при GET запросе `/api/v1/users/{username}/` с токеном авторизации возвращается статус 200'
        response_data = response.json()
    ...
    def test_07_users_username_patch_auth(self, user_client, admin):
        ...
        response = user_client.patch(f'/api/v1/users/{admin.email}/', data=data)
        assert response.status_code == 200, \
        ...
        assert test_admin.last_name == data['last_name'], \
            'Проверьте, что при PATCH запросе `/api/v1/users/{username}/` изменяете данные.'
        response = user_client.patch(f'/api/v1/users/{user.email}/', data={'role': 'admin'})
        assert response.status_code == 200, \
            'Проверьте, что при PATCH запросе `/api/v1/users/{username}/` с токеном авторизации возвращается статус 200'
        client_user = auth_client(user)
        response = client_user.get(f'/api/v1/users/{admin.email}/')
        assert response.status_code == 200, \
    ...
    def test_08_users_username_delete_auth(self, user_client):
        user, moderator = create_users_api(user_client)
        response = user_client.delete(f'/api/v1/users/{user.email}/')
    ...
    @pytest.mark.django_db(transaction=True)
    def test_11_users_me_put(self, user_client):
        user, moderator = create_users_api(user_client)
        data = {
            'first_name': 'Admin',
            'last_name': 'Test',
        ...
        response = user_client.put(f'/api/v1/users/me/', data=data)
        assert response.status_code == 200, \
        ...
        client_user = auth_client(moderator)
        response = client_user.put(f'/api/v1/users/me/', data={'first_name': 'NewTest'})
