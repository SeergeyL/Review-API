...
class Test03GenreAPI:
    ...
    def test_02_genre(self, user_client):
        ...
        assert type(data) == list, \
            'Проверьте, что при GET запросе `/api/v1/genres/` возвращаете список. '
        assert len(data) == 2, \
            'Проверьте, что при GET запросе `/api/v1/genres/` возвращаете правильное количество данных. '
        assert {'name': 'Ужасы', 'slug': 'horror'} in data, \
            'Проверьте, что при GET запросе `/api/v1/genres/` возвращаете верные записи.' \
