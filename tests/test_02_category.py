...
class Test02CategoryAPI:
    ...
    def test_02_category(self, user_client):
        ...
        assert type(data) == list, \
            'Проверьте, что при GET запросе `/api/v1/categories/` возвращаете тип данных список'

        assert len(data) == 2, \
            'Проверьте, что при GET запросе `/api/v1/categories/` возвращаете верное количество данных. '

        assert {'name': 'Книги', 'slug': 'books'} in data, \
            'Проверьте, что при GET запросе `/api/v1/categories/` выводятся правильные значения.'
